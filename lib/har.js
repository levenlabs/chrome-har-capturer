...
function fromPage(page) {
    ...
    for (var requestId in page.objects) {
        ...
        var cookies;
        if (object.requestCookies) {
            cookies = object.requestCookies.map(function (c) {
                var expires = (new Date(c.expires * 1000)).toISOString();
                return Object.assign({}, c, { expires: expires });
            });
        }
        // fill entry
        entries.push({
