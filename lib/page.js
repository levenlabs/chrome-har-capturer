...
Page.prototype.hasPendingResources = function () {
    var self = this;
    for (var id in self.objects) {
        if (!self.objects[id].failed && !self.objects[id].canceled &&
            !self.objects[id].responseFinished &&
            !(self.fetchContent && self.objects[id].responseBody)
            ) {
            return true;
        }
    }
    ...
    default:
        if (message.method.match(/^Network\./)) {
            ...
            case 'Network.requestWillBeSent':
                ...
                self.chrome.Network.getCookies({ urls: [message.params.request.url] }, function (error, response) {
                    if (response && (id in self.objects)) {
                        self.objects[id].requestCookies = response.cookies;
                    }
                });
                break;
            ...
            case 'Network.loadingFailed':
                ...
                if (id in this.objects) {
                    if (message.params.canceled) {
                        this.objects[id].canceled = true;
                    } else {
                        this.objects[id].failed = true;
                        this.objects[id].failedReason = message.params.blockedReason || message.params.errorText;
                    }
                }
                break;
            }
